...
def calendar_url(base_url: str = DEFAULT_BASE_URL, giornata: int | None = None) -> str:
    path = "serie-a/calendario" + (f"/{int(giornata)}" if giornata is not None else "")
    return urljoin(base_url.rstrip("/") + "/", path)


def parse_calendar_page(html: str, base_url: str = DEFAULT_BASE_URL) -> pd.DataFrame:
    soup = BeautifulSoup(html, "html.parser")
    panel = soup.select_one("#panel-2 #calendar") or soup.select_one("#calendar")
    rows, seasons = [], set()
    if panel is None:
        return pd.DataFrame(columns=["giornata"])
    for match in panel.select('.match-pill[itemtype="http://schema.org/SportsEvent"]'):
        mw = match.select_one(".matchweek")
        giornata = _num(mw.get_text(" ", strip=True)) if mw else None
        if not isinstance(giornata, int):
            continue
        home = _meta_content(match, '[itemprop="homeTeam"] meta[itemprop="name"]')
        away = _meta_content(match, '[itemprop="awayTeam"] meta[itemprop="name"]')
        link = match.select_one("a.match-score[href]")
        href = link.get("href") if link else _meta_content(match, 'meta[itemprop="url"]')
        if href:
            found = re.search(r"/calendario/\d+/(\d{4}-\d{2})/", href)
            if found: seasons.add(found.group(1))
        hours = match.select_one(".match-date .hours")
        stadium = match.select_one('.match-location [itemprop="location"]')
        rows.append({
            "giornata": giornata,
            "match_url": urljoin(base_url.rstrip("/") + "/", href) if href else None,
            "match_text": f"{home} - {away}" if home and away else None,
            "team_home": home, "team_away": away,
            "match_date": _meta_content(match, 'meta[itemprop="startDate"]'),
            "match_time": _clean(hours.get_text(" ", strip=True)) if hours else None,
            "stadium": _clean(stadium.get_text(" ", strip=True)) if stadium else None,
            "calendar_match_status": _num(match.get("data-match-status")),
        })
    df = pd.DataFrame(rows)
    if not df.empty:
        df = df.drop_duplicates(["giornata","team_home","team_away"]).sort_values(["giornata","match_date","match_time"], na_position="last").reset_index(drop=True)
    df.attrs["season"] = next(iter(seasons)) if len(seasons) == 1 else None
    return df


def expected_matchweeks_from_price_graph(player_df: pd.DataFrame) -> set[int]:
    """Positive matchweeks published on the quotation-graph x-axis."""
    result: set[int] = set()
    for value in player_df.attrs.get("price_graph_matchweeks", []):
        numeric = pd.to_numeric(value, errors="coerce")
        if pd.notna(numeric) and int(numeric) > 0:
            result.add(int(numeric))
    if result:
        return result
    if "giornata" not in player_df.columns:
        return set()
    return set(pd.to_numeric(player_df["giornata"], errors="coerce").dropna().astype(int))


def _placeholder_from_fixture(fixture: Mapping[str, Any], team: str) -> dict[str, Any]:
    row = {column: None for column in PLAYER_MATCH_COLUMNS}
    for column in ("giornata","match_url","match_text","team_home","team_away","match_date","match_time","stadium","calendar_match_status"):
        row[column] = fixture.get(column)
    row.update({
        "status":"scheduled", "status_code":fixture.get("calendar_match_status"),
        "active_team":team, "events":[], "event_counts":{},
        ...
        "record_type":"calendar_placeholder", "played":False, "is_placeholder":True,
    })
    return row
...
def resolve_calendar_team(
    ...
    """Resolve the full calendar team name, preferring player-page metadata.

    The quotation table may use abbreviations such as ``Int`` while the
    calendar uses ``Inter``. Player-page metadata and team URLs normally use
    the full identity and must therefore take precedence.
    """
    candidates = [
        player_df.attrs.get("team"),
        player_df.attrs.get("bridge", {}).get("teamName")
        if isinstance(player_df.attrs.get("bridge"), Mapping) else None,
        quotation_team,
    ]
    ...
    for column in ("team_home", "team_away"):
        ...
        for team in calendar_df[column].dropna().astype(str):
            known[_clean(team).casefold()] = team

    for candidate in candidates:
        if candidate is None or pd.isna(candidate):
            continue
        key = _clean(str(candidate)).casefold()
        if key in known:
            return known[key]

    # Conservative abbreviation match. Accept only a unique prefix.
    for candidate in candidates:
        if candidate is None or pd.isna(candidate):
            continue
        key = _clean(str(candidate)).casefold()
        matches = [full for normalised, full in known.items() if normalised.startswith(key)]
        if len(set(matches)) == 1:
            return matches[0]
    return None


def add_calendar_placeholders(player_df: pd.DataFrame, calendar_df: pd.DataFrame, team: str | None, expected_matchweeks: set[int] | None = None) -> pd.DataFrame:
    attrs = dict(player_df.attrs)
    actual = player_df.reindex(columns=PLAYER_MATCH_COLUMNS).copy()
    if not team or calendar_df.empty:
        actual.attrs.update(attrs); return actual
    key = _clean(str(team)).casefold()
    home = calendar_df["team_home"].fillna("").astype(str).map(lambda x: _clean(x).casefold())
    away = calendar_df["team_away"].fillna("").astype(str).map(lambda x: _clean(x).casefold())
    ...
    if expected_matchweeks is not None:
        target = {int(g) for g in expected_matchweeks if int(g) > 0}
    else:
        target = {
            int(x)
    ...
    schedule_by_giornata = {
        int(r["giornata"]): r
        for r in schedule.to_dict("records")
    }
    placeholders = [
        _placeholder_from_fixture(
            schedule_by_giornata[g],
            str(team)
        )
        for g in sorted(missing)
        if g in schedule_by_giornata
    ]
    result = pd.concat([actual, pd.DataFrame(placeholders, columns=PLAYER_MATCH_COLUMNS)], ignore_index=True) if placeholders else actual
    metadata = schedule[["giornata","match_date","match_time","stadium","calendar_match_status"]].drop_duplicates("giornata")
    result = result.drop(columns=["match_date","match_time","stadium","calendar_match_status"]).merge(metadata, on="giornata", how="left")
    result = result.reindex(columns=PLAYER_MATCH_COLUMNS).sort_values("giornata").reset_index(drop=True)
    result.attrs.update(attrs)
    result.attrs["calendar_season"] = calendar_df.attrs.get("season")
    result.attrs["calendar_placeholders_added"] = len(placeholders)
    return result
...
class CalendarScraper:
    def __init__(self, client: FantacalcioHTTPClient, base_url: str, max_workers: int):
        self.client, self.base_url, self.max_workers = client, base_url, max_workers

    def scrape(self) -> pd.DataFrame:
        root = BeautifulSoup(self.client.get_text(calendar_url(self.base_url)), "html.parser")
        giornate = sorted({int(a.get_text(" ", strip=True)) for a in root.select("#panel-2 .list-weeks a[href]") if a.get_text(" ", strip=True).isdigit()}) or [1]
        def worker(giornata):
            return parse_calendar_page(self.client.get_text(calendar_url(self.base_url, giornata)), self.base_url)
        with ThreadPoolExecutor(max_workers=min(self.max_workers, len(giornate))) as pool:
            frames = list(pool.map(worker, giornate))
        valid = [f for f in frames if not f.empty]
        if not valid: return pd.DataFrame(columns=["giornata"])
        calendar = pd.concat(valid, ignore_index=True).drop_duplicates(["giornata","team_home","team_away"]).sort_values(["giornata","match_date","match_time"], na_position="last").reset_index(drop=True)
        seasons = {f.attrs.get("season") for f in valid if f.attrs.get("season")}
        calendar.attrs["season"] = next(iter(seasons)) if len(seasons) == 1 else None
        return calendar
...
class FantacalcioRunner:
    ...
    def __init__(self, config: ScraperConfig):
        ...
        self.calendar_scraper = CalendarScraper(self.client, config.base_url, config.max_workers)
    ...
    def load_or_scrape_calendar(self, force: bool = False) -> pd.DataFrame:
        ...
        print(f"Scraping current Serie A calendar: {calendar_url(self.config.base_url)}")
        calendar = self.calendar_scraper.scrape()
    ...
    def load_or_scrape_stats(self, quotazioni: pd.DataFrame, force: bool = False) -> dict[str, pd.DataFrame]:
        ...
        if calendar.attrs.get("season") != self.config.season:
            print(f"Current calendar season {calendar.attrs.get('season')!r} does not match requested season {self.config.season!r}; placeholders disabled")
            calendar = pd.DataFrame(columns=["giornata"])
        print(f"Scraping player stats with {self.config.max_workers} workers")
        start = time_s()
        stats = self.scrape_stats_parallel(quotazioni, calendar)
        for name, df in stats.items():
            expected = expected_matchweeks_from_price_graph(df)
            present = set(pd.to_numeric(df["giornata"], errors="coerce").dropna().astype(int))
            unresolved = expected - present
            if unresolved and not calendar.empty:
                raise RuntimeError(
                    f"{name}: expected matchweeks missing from stats JSON: {sorted(unresolved)}; "
                    f"calendar_team={df.attrs.get('calendar_team_resolved')!r}, "
                    f"quotation_team={df.attrs.get('quotation_team')!r}, "
                    f"player_team={df.attrs.get('team')!r}"
                )
    ...
    def scrape_stats_parallel(self, quotazioni: pd.DataFrame, calendar: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        rows = quotazioni[["nome", "url", "squadra"]].dropna(subset=["nome", "url"]).to_dict("records")
        stats: dict[str, pd.DataFrame] = {}

        def worker(row: Mapping[str, Any]) -> tuple[str, pd.DataFrame]:
            df = self.player_scraper.scrape(str(row["url"]))
            expected_matchweeks = expected_matchweeks_from_price_graph(df)
            team = resolve_calendar_team(df, row.get("squadra"), calendar)
            completed = add_calendar_placeholders(
                df,
                calendar,
                team,
                expected_matchweeks=expected_matchweeks,
            )
            completed.attrs["calendar_team_resolved"] = team
            completed.attrs["quotation_team"] = row.get("squadra")
            return str(row["nome"]), completed
