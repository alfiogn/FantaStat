...
async function loadPlayer() {
  ...
  currentPlayerNoteId = String(p.player_id || id);
...
days`].filter(Boolean).join(' | ');
  ...
  renderBoxplots(s);
  await renderPlayerNotes(currentPlayerNoteId, p.name || id);
}

async function renderPlayerNotes(playerId, playerName) {
    const section = ensurePlayerNotesSection();
    const payload = await Fantastat.getJSON(`/api/notes/${encodeURIComponent(playerId)}`);
    section.querySelector('[data-note-player]').textContent = playerName;
    section.querySelector('[data-note-text]').value = payload.note || '';
}

function ensurePlayerNotesSection() {
    let section = document.getElementById('playerNotesPanel');
    if (section) return section;

    section = document.createElement('section');
    section.id = 'playerNotesPanel';
    section.className = 'panel';
    section.innerHTML = `
        <div class="panel-header">
            <h2>Notes</h2>
            <span class="muted" data-note-player></span>
        </div>
        <textarea class="input" data-note-text rows="5" style="width:100%; resize:vertical;"></textarea>
    ...
    section.querySelector('[data-save-note]').addEventListener('click', async () => {
        const note = section.querySelector('[data-note-text]').value.trim();
        await fetch(`/api/notes/${encodeURIComponent(currentPlayerNoteId)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ note }),
        });
        Fantastat.toast('Note saved');
    });
...
function renderBoxplots(summary) {
  const b = summary.window_boxplots || {};
